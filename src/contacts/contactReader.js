...
function parseCSV(raw) {
  ...
  for (const col of REQUIRED_COLUMNS) {
    if (!headers.includes(col)) {
      throw new Error(`Columna requerida ausente: "${col}". Encontradas: ${headers.join(', ')}`);
    }
  }
...
function validateRow(contact) {
  ...
  if (!tipo) return `Fila ${contact._row}: falta "tipo"`;
  if (!types[tipo]) return `Fila ${contact._row}: tipo desconocido "${contact.tipo}"`;
  return null;
}
